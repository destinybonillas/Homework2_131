...
std::size_t BookList::find( const Book & book ) const
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    // loop through to return the book, else size is empty and just return size
    for (unsigned long i = 0; i < _books_vector.size(); ++i) {
      if (_books_vector.at(i) == book) {
        return std::distance( _books_vector.begin(), std::find( _books_vector.begin(), _books_vector.end(), book ) );
      }
    }
    return _books_vector.size();
    
    //return std::distance( _books_array.begin(), std::find( _books_array.begin(), _books_array.end(), book ) );
...
void BookList::insert( const Book & book, std::size_t offsetFromTop )       // insert new book at offsetFromTop, which places it before the current book at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      // to insert shift everything after insertion point
        std::move_backward( &_books_array[ offsetFromTop ], _books_array.end()-1, _books_array.end() );
      } else {
        throw CapacityExceeded_Ex("Capacity has been reached");
      }
      // perform insert
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ///////////////////////// TO-DO (5) //////////////////////////////
      ...
      auto it = _books_vector.begin();
      // create other iterator to hold new position
      auto iter = std::next( it, offsetFromTop );

      // insert into vector
      _books_vector.insert( iter, book );
      // convert index to iterator and double capacity if needed
      /*std::size_t index = offsetFromTop - _books_vector.begin();
      reserve( _books_vector.capacity() == 0 ? 8 : 2 * _books_vector.capacity() );
      offsetFromTop = _books_vector.begin() + index;*/
      //std::cout << _books_vector.size() << "\n";
    /////////////////////// END-TO-DO (5) ////////////////////////////
  } // Part 2 - Insert into vector
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
    ///////////////////////// TO-DO (6) //////////////////////////////
      ...
      auto it = _books_dl_list.begin();
      auto iter = std::next( it, offsetFromTop );

      // insert into the DLL
      _books_dl_list.insert( iter, book );

      //std::cout << _books_dl_list.size() << "\n";
    /////////////////////// END-TO-DO (6) ////////////////////////////
  } // Part 3 - Insert into doubly linked list
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      auto it = _books_sl_list.before_begin();
      auto iter = std::next( it, offsetFromTop );

      // insert into the SLL
      _books_sl_list.insert_after( iter, book );

      //std::cout << _books_sl_list.max_size() << "\n";
    /////////////////////// END-TO-DO (7) ////////////////////////////
  } // Part 4 - Insert into singly linked list
...
void BookList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      // to erase close hole and move left
...
void BookList::moveToTop( const Book & book )
{
  ///////////////////////// TO-DO (12) //////////////////////////////
    ...
    // if found not equal to size then remove and insert
    if ( find( book ) != size() ) {
      remove( book );
      insert( book, 0 );
    }
...
BookList & BookList::operator+=( const BookList & rhs )
{
  ///////////////////////// TO-DO (14) //////////////////////////////
    ...
    for ( size_t i = 0; i < rhs._books_vector.size(); ++i ) {
      BookList::insert( rhs._books_vector[i], Position::BOTTOM );
      //return *this;
    }
...
std::weak_ordering BookList::operator<=>( BookList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    auto it = _books_array.begin();

    auto const extent = _books_array.size() < rhs.size() ? _books_array.size() : rhs.size();

    for ( auto i = it, end = i + extent, j = rhs._books_array.begin(); i != end; ++i, ++j ) {
      auto result = std::compare_weak_order_fallback( *i, *j );
      if ( result != 0 ) return result;
    }
    return _books_array.size() <=> rhs.size();
...
bool BookList::operator==( BookList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    auto it = _books_array.begin();
    auto iter = _books_array.end();

    if ( _books_array_size != rhs.size() ) {
      return false;
    }

    for ( auto i = it, end = iter, j = rhs._books_array.begin(); i != end; ++i, ++j ) {
      if ( *i != *j ) return false;
    }
