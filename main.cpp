  ...
  try
  {
    ...
    ///////////////////////// TO-DO (1) //////////////////////////////
     ...
     /// lists.  Have some fun with it!  The purpose is to show me you as a client understand how to *use* the BookList.
      ...
      // Let's start a book list
    BookList booksBeingRead = {{"Diary of a Wimpy Kid"           },
                            {"Moby Dick"              },
                            {"To Kill a Mockingbird"                },
                            {"Spiderman"   }};

    // Changed my mind, I want to make sure I read Les Mis first so I'm going to move that to the top of my list
    booksBeingRead.moveToTop( {"To Kill a Mockingbird"} );

    // Let's see what's on the list so far
    std::cout << "My Book List" << booksBeingRead << "\n\n";

    // Yay, no.  Need to add a few more
    booksBeingRead += {{"Divergent"          },
                    {"The Alchemist" }};
    ...
    // Let's combine the lists (if different) and go shopping
    if( booksBeingRead != roommatesList ) booksBeingRead += roommatesList;


    // Oops, forgot one of my favorites.  Let's add that right before '2 cities.
    booksBeingRead.insert( {"Oliver Twist", "Charles Dickens"}, booksBeingRead.find( {"Spiderman"} ) );

    // Time is short, let me take one off the list!
    booksBeingRead.remove( {"Harry Potter", "J. K. Rowling"} );


    // Now let's see what's on the list
    std::cout << "Combined Book Lists" << booksBeingRead << "\n\n";



    // Did you get it right?
    BookList expected = {{"To Kill a Mockingbird"                         },
    ...
    std::cout << "\nExpected results:" << expected                                        << "\n\n"
              << "\nActual results:"   << booksBeingRead                                            << "\n\n"
              << "\nTest results:  "   << ( booksBeingRead == expected  ? "PASS"  :  "FAIL") << '\n';
